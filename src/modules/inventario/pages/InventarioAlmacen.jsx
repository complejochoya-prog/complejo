...
import { Archive, ArrowLeft, Search, Plus, Filter, X } from 'lucide-react';
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <div className="flex items-center gap-3">
                    <div className="relative min-w-[200px] hidden md:block">
                        ...
                        <input type="text" placeholder="Buscar producto..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                            className="w-full bg-slate-900 border border-white/5 rounded-xl py-3 pl-11 pr-4 text-xs font-bold text-white focus:outline-none focus:border-indigo-500/50 transition-all" />
                    ...
                    <button onClick={() => { setTargetProduct(null); setShowForm(true); }}
                        className="flex items-center gap-2 bg-indigo-500 hover:bg-indigo-400 text-white px-5 py-3 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all shadow-xl shadow-indigo-500/20">
            ...
            {productos.length > 0 ? (
                ...
                <div className="py-20 text-center space-y-4 bg-slate-900/30 rounded-[32px] border border-dashed border-white/[0.06]">
                    ...
                    <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">No se encontraron ítems en Almacén</p>
                </div>
            )}
