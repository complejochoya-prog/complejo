...
export default function KitchenDashboard() {
    ...
    useEffect(() => {
        ...
        if (employeeUser.role !== 'cocina' && employeeUser.role !== 'admin') {
            return navigate(`/${negocioId}/empleados/${employeeUser.role}`);
        }

        const init = async () => {
            const data = await fetchEmployeeOrders(negocioId, 'cocina');
            setOrders(data);
            setLoading(false);
        };
        init();
        ...
        <div className="min-h-screen bg-slate-950 flex flex-col animate-in slide-in-from-right-8 duration-500">
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 max-w-7xl mx-auto w-full">
                <header className="flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-black uppercase tracking-tighter italic text-orange-500">
                            Monitor Cocina KDS
                        </h2>
                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">
                            Comandas Activas: {pending.length + preparing.length}
                        </p>
                    </div>
                </header>
                ...
                <div className="flex flex-col xl:flex-row gap-8">
                    {/* Columna Pendientes */}
                    <div className="flex-1 bg-slate-900/30 border border-orange-500/10 rounded-[32px] p-6">
                        <h3 className="text-sm text-orange-500 font-black uppercase tracking-widest flex items-center gap-2 mb-6">
                            Nuevas Comandas 
                            <span className="bg-orange-500/20 px-2 py-0.5 rounded text-orange-400">{pending.length}</span>
                        </h3>
